...
class AssetsApi {
  ...
  /// Parameters:
  ///
  /// * [CheckExistingAssetsByMetadataDto] checkExistingAssetsByMetadataDto (required):
  Future<Response> checkExistingAssetsByMetadataWithHttpInfo(CheckExistingAssetsByMetadataDto checkExistingAssetsByMetadataDto,) async {
    // ignore: prefer_const_declarations
    final apiPath = r'/assets/exist/metadata';

    // ignore: prefer_final_locals
    Object? postBody = checkExistingAssetsByMetadataDto;
  ...
  ///
  /// * [CheckExistingAssetsByMetadataDto] checkExistingAssetsByMetadataDto (required):
  Future<CheckExistingAssetsByMetadataResponseDto?> checkExistingAssetsByMetadata(CheckExistingAssetsByMetadataDto checkExistingAssetsByMetadataDto,) async {
    final response = await checkExistingAssetsByMetadataWithHttpInfo(checkExistingAssetsByMetadataDto,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'CheckExistingAssetsByMetadataResponseDto',) as CheckExistingAssetsByMetadataResponseDto;
