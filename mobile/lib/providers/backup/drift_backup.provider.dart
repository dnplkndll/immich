...
class DriftBackupNotifier extends StateNotifier<DriftBackupState> {
  DriftBackupNotifier(this._foregroundUploadService, this._backgroundUploadService, this._backupRepository, this._uploadSpeedManager)
    : super(
        const DriftBackupState(
          totalCount: 0,
          backupCount: 0,
  ...
  void _handleForegroundBackupSuccess(String localAssetId, String remoteAssetId) {
    ...
    _backupRepository.storeRemoteId(localAssetId, remoteAssetId).catchError(
      (e) => _logger.warning('Failed to store remoteId for $localAssetId', e),
    );
