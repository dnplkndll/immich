...
class DriftBackupRepository extends DriftDatabaseRepository {
  ...
  /// Returns metadata for backup-selected assets not yet confirmed on server.
  /// Includes unhashed (NULL checksum) and hashed-but-unmatched rows,
  /// excluding sentinel-marked assets.
  Future<List<({String id, String name, DateTime createdAt, int width, int height})>> getUnmatchedBackupAssetMetadata(
    String userId,
  ) async {
    const sql = '''
