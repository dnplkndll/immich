...
describe('duplicate utils', () => {
  ...
  describe('suggestDuplicate', () => {
    ...
    it('should handle fileSize = 0 without NaN', () => {
      ...
      zeroSize.exifInfo = ExifResponseSchema.parse({ fileSizeInByte: 0 });
    ...
    it('should prefer 1080p video over re-encoded 720p copy (skip compression efficiency for videos)', () => {
      const original = createAsset('original', {
        type: AssetType.Video,
        fileSizeInByte: 27_000_000,
        exifImageWidth: 1920,
        exifImageHeight: 1080,
      });
      const reencode = createAsset('reencode', {
        type: AssetType.Video,
      ...
      expect(suggestDuplicate([original, reencode])?.id).toBe('original');
      expect(suggestDuplicate([reencode, original])?.id).toBe('original');
    });

    it('should prefer higher-bitrate video when resolution is the same', () => {
      const highBitrate = createAsset('high-bitrate', {
        type: AssetType.Video,
        fileSizeInByte: 50_000_000,
        exifImageWidth: 1920,
        exifImageHeight: 1080,
      });
      const lowBitrate = createAsset('low-bitrate', {
        type: AssetType.Video,
      ...
      expect(suggestDuplicate([highBitrate, lowBitrate])?.id).toBe('high-bitrate');
    });
  });
